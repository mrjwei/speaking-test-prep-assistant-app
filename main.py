...
class SpeakingAssistant(Frame):
    def __init__(self, root, **kwargs):
        ...
        style.configure('TLabel', font=('Helvetica', 20), padding=[0, 5])
        style.configure('Alert.TLabel', foreground='red')
        style.configure('TButton', font=('Helvetica', 20), padding=[0, 10])
        style.configure('TEntry', padding=[0, 10])
        style.configure('TCombobox', font=('Helvetica', 20), padding=[0, 10])
        ...
        self.mode_select_frame.grid(row=0, column=0, columnspan=4, sticky='we', padx=20, pady=20)
        self.mode_select_frame.columnconfigure(0, weight=1)
        self.mode_select_frame.columnconfigure(1, weight=1)
        self.mode_select_frame.columnconfigure(2, weight=1)
        self.mode_select_frame.columnconfigure(3, weight=1)
        ...
        self.test_part_select_frame.grid(row=1, column=0, columnspan=4,  sticky='we', padx=20)
        self.test_part_select_frame.columnconfigure(0, weight=1)
        self.test_part_select_frame.columnconfigure(1, weight=1)
        self.test_part_select_frame.columnconfigure(2, weight=1)
        self.test_part_select_frame.columnconfigure(3, weight=1)

        Label(self.test_part_select_frame, text='Select test part:').grid(
            row=0, column=0, sticky=W, padx=5)

        self.test_part_select = Combobox(
            self.test_part_select_frame, values=list(PARTS.keys()))
        self.test_part_select.grid(
            row=1, column=0, columnspan=4, sticky='we', padx=5)
        self.test_part_select.bind(
            '<<ComboboxSelected>>', self._on_select_test_part)

        self.alert_msg_test_part = Label(
            self.test_part_select_frame, text='You must choose a part.', style='Alert.TLabel')

        # question entry widgets group
        self.question_frame = Frame(self.main_frame)
        self.question_frame.grid(row=2, column=0, columnspan=4, sticky='we', padx=20, pady=20)
        ...
        Label(self.question_frame, text='Question:').grid(
            row=0, column=0, sticky=W, padx=5)

        self.question_entry = Entry(self.question_frame)
        self.question_entry.grid(
            row=1, column=0, columnspan=3, sticky='we', padx=5)
        self.question_entry.bind('<KeyPress>', self._on_key_press)

        self.question_record_btn = Button(
            self.question_frame, text='Record', command=self.record_question)
        self.question_record_btn.grid(row=1, column=3, sticky='we', padx=5)

        self.alert_msg_question = Label(
            self.question_frame, text='Question cannot be empty.', style='Alert.TLabel')

        # answer widgets group
        self.answer_frame = Frame(self.main_frame)
        self.answer_frame.grid(row=3, column=0, columnspan=4, sticky='we', padx=20)
        ...
        self.answer_frame.columnconfigure(3, weight=1)

        Label(self.answer_frame, text='Answer:').grid(
            row=0, column=0, sticky=W, padx=5)

        self.filepath_btn = Button(
            self.answer_frame, text='Select Audio File', command=self.open_file_browser)
        self.filepath_btn.grid(
            row=1, column=0, columnspan=3, sticky='we', padx=5)

        self.answer_record_btn = Button(
            self.answer_frame, text='Record', command=self.record_answer)
        self.answer_record_btn.grid(row=1, column=3, sticky='we', padx=5)

        self.answer_label = Label(self.answer_frame, text='', wraplength=400)
        self.answer_label.grid(row=2, column=0, columnspan=4, padx=5)

        self.alert_msg_answer = Label(
            self.answer_frame, text='Answer cannot be empty.', style='Alert.TLabel')

        # button widgets group
        self.buttons_frame = Frame(self.main_frame)
        self.buttons_frame.grid(row=4, column=0, columnspan=4, sticky='we', padx=20, pady=20)
        self.buttons_frame.columnconfigure(0, weight=1)
        self.buttons_frame.columnconfigure(1, weight=1)
        self.buttons_frame.columnconfigure(2, weight=1)
        self.buttons_frame.columnconfigure(3, weight=1)

        self.gen_btn = Button(self.buttons_frame,
                              text='Generate Text', command=self.generate_text)
    ...
    def _answer_callback(self, transcript):
        self.answer_label.configure(text=f'{transcript[:100]}...')
        ...
        try:
            ...
            completion = client.chat.completions.create(
                ...
                messages=[
                    {
                      'role': 'system',
                      'content': f'This is {self.test_part} of a practice IELTS speaking test. The question is: {self.question} And following is my answer. Can you revise it for 8.5-point score or higher? Please keep your length similar to mine, keep the overall tone conversational rather than sounding too formal, use UK English instead of US English and please do not provide any other text in your response than the revised answer.'
                    },
                    {
                        'role': 'user',
            ...
            data = {
                "children": [
                    api.create_block_object(
                        "heading_2", ("text", self.question.capitalize())),
                    api.create_block_object(
                        "heading_3", ("text", "My answer:")),
                    api.create_block_object(
                        "paragraph", ("text", self.original_answer)),
                    api.create_block_object(
                        "heading_3", ("text", "ChatGPT answer:")),
                    api.create_block_object(
                        "paragraph", ("text", self.revised_answer)),
                ]
            }
